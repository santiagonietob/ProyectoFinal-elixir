defmodule HackathonApp.Service.ProyectoServicio do
  ...
  CSV:
    - data/proyectos.csv   id,equipo_id,titulo,categoria,estado,fecha_registro
    - data/avances.csv     id,proyecto_id,contenido,fecha_iso
  """
  ...
  def crear(equipo_id, titulo, categoria) do
    ...
    fila = [to_string(id), to_string(equipo_id), limpiar(titulo), limpiar(categoria), "idea", ahora]
    :ok = CSV.agregar(@ruta_proy, fila)
    {:ok, %Proyecto{id: id, equipo_id: equipo_id, titulo: titulo, categoria: categoria, estado: "idea", fecha_registro: ahora}}
  end
  ...
  def listar do
    ...
    |> Enum.map(fn [id, eq, t, c, e, f] ->
      %Proyecto{id: String.to_integer(id), equipo_id: String.to_integer(eq), titulo: t, categoria: c, estado: e, fecha_registro: f}
    end)
  end

  @spec buscar_por_equipo(String.t()) :: Proyecto.t() | nil
  def buscar_por_equipo(nombre_equipo) do
    # Reusa tu EquipoServicio para resolver equipo_id por nombre
  ...
  def cambiar_estado(proyecto_id, nuevo_estado) when nuevo_estado in ["idea","en_progreso","entregado"] do
    proyectos = listar()
    case Enum.find(proyectos, &(&1.id == proyecto_id)) do
      nil -> {:error, "Proyecto no existe"}
      %Proyecto{} = p ->
        actualizado = %Proyecto{p | estado: nuevo_estado}
        reescribir_proyectos(reemplazar(proyectos, actualizado))
        {:ok, actualizado}
    end
  end

  def cambiar_estado(proyecto_id, nuevo_estado) do
  estados = ["idea", "en_progreso", "entregado"]

  if nuevo_estado in estados do
  ...
  def agregar_avance(proyecto_id, contenido) do
    if proyecto?(proyecto_id) do
      ...
      fila = [to_string(id), to_string(proyecto_id), limpiar(contenido), fecha]
      :ok = CSV.agregar(@ruta_av, fila)
      avance = %Avance{id: id, proyecto_id: proyecto_id, contenido: contenido, fecha_iso: fecha}

      # Notificación "tiempo real" (Adapter.ProyectoStream)
  ...
  def listar_avances(proyecto_id) do
    CSV.leer(@ruta_av)
    |> Enum.filter(fn [_id, p, _c, _f] -> p == to_string(proyecto_id) end)
    |> Enum.map(fn [id, p, c, f] -> %Avance{id: String.to_integer(id), proyecto_id: String.to_integer(p), contenido: c, fecha_iso: f} end)
  end
  ...
  defp reescribir_proyectos(proys) do
    filas =
      proys
      |> Enum.map(fn p -> [to_string(p.id), to_string(p.equipo_id), p.titulo, p.categoria, p.estado, p.fecha_registro] end)
  ...
  defp siguiente_id_proyecto, do: case listar() do [] -> 1; xs -> Enum.max_by(xs, & &1.id).id + 1 end
  defp siguiente_id_avance do
    CSV.leer(@ruta_av)
    |> Enum.map(fn [id | _] -> String.to_integer(id) end)
    |> case do [] -> 1; xs -> Enum.max(xs) + 1 end
  end
