defmodule HackathonApp.Service.EquipoServicio do
  ...
  - Crear equipo (por tema/afinidad)
  - Listar equipos y activos
  ...
  def crear_equipo(nombre, descripcion, tema, activo \\ true) do
    if Enum.any?(listar_todos(), &(&1.nombre == nombre)) do
      ...
      {:ok, %Equipo{id: id, nombre: nombre, descripcion: descripcion, tema: tema, activo: activo}}
    end
  end
  ...
  def listar_equipos_activos do
    listar_todos() |> Enum.filter(& &1.activo)
  end
  ...
  def unirse_a_equipo(nombre_equipo, usuario_id, rol_en_equipo \\ "miembro") do
    with %Equipo{} = equipo <- buscar_equipo_por_nombre(nombre_equipo),
      ...
      nil -> {:error, "No existe el equipo #{nombre_equipo}"}
      true -> {:error, "El usuario ya pertenece a #{nombre_equipo}"}
    end
  end
  ...
  def listar_miembros(nombre_equipo) do
    case buscar_equipo_por_nombre(nombre_equipo) do
      ...
      %Equipo{id: eq_id} ->
        CSV.leer(@ruta_membres)
        |> Enum.filter(fn [u_id, e_id, _] -> e_id == Integer.to_string(eq_id) end)
  ...
  def crear_equipo_por_afinidad(nombres_participantes, tema, descripcion \\ "") do
    ...
    with {:ok, equipo} <- crear_equipo(nombre_equipo, descripcion, tema) do
      Enum.each(nombres_participantes, fn nombre ->
